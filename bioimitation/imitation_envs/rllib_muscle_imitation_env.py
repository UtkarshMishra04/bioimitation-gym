...
class MuscleImitationEnv(OsimEnv):
    def __init__(self,
                 config
                 ):
        ...
        integration_accuracy = config['integration_accuracy']
        visualize = config['visualize']
        q_desired_file = config['q_desired_file']
        u_desired_file = config['u_desired_file']
        body_pos_desired_file = config['body_pos_desired_file']
        body_vel_desired_file = config['body_vel_desired_file']
        body_vel_desired_file = config['body_vel_desired_file']
        self.max_actuation = config['max_actuation']

        construct_predictive_model(model_input_file, model_file)

        if config['use_torque_actuation']:
            convert_model_to_torque_actuated(model_file, model_file, self.max_actuation)

        if config['prosthetic']:
        ...
        if self.task == 'walking':
            if self.test:
                ...
                self.N = config['N_steps']  # Calculated steps for completion of one gait cycle

        if self.task == 'highjump':
            if self.test:
                self.N = 2*(self.q_d.shape[0] - 2)  # N is used to terminate the simulation
            else:
                self.N = 2*(self.q_d.shape[0] - 2)  # Calculated steps for completion of one gait cycle
    ...
    def convert_model_to_prosthetic(self, model_file, model_file_out):
        ...
        for i in range(muscle_names.getSize()):
            if muscle_names.get(i) in ['gastroc_l','soleus_l','tib_ant_l']:
    ...
    def step(self, action, obs_as_dict=False):
        ...
        if self.osim_model.model.getMuscles().getSize() == 0:
        ...
        return super(MuscleImitationEnv, self).step(action, obs_as_dict)
    ...
    def reset(self, obs_as_dict=False):
        ...
        if self.test:
            ...
            index = random.randint(
                0, self.N/2)  # it seems with this it is easier to learn
    ...
    def get_state_dict(self):
        ...
        smaller.'''
        state_dict = {}
        state_dict.update(self.osim_model.calc_joint_kinematics())
        state_dict.update(self.osim_model.calc_body_kinematics())
        state_dict.update(self.osim_model.calc_muscles_info())
        ...
        if self.task == 'highjump':
            self.cycle = self.N/2  # Calculated steps for completion of one gait cycle
        ...
        if self.osim_model.is_muscle_model:
            observation['muscles'] = {}
    ...
    def is_done(self):
        ...
        '''
        observations = self.get_observation_dict()
    ...
    def get_reward(self):
        ...
        if self.task == 'walking' or self.task == 'running':
            index = self.osim_model.istep

        if self.task == 'highjump':
            if self.osim_model.istep <= self.cycle:
                index = self.osim_model.istep
            else:
                index = int(2*self.cycle - self.osim_model.istep)
        ...
        if self.osim_model.is_muscle_model:
            ...
            effort = self.calc_cost_of_transport()/2000
        else:
            a_error = 0.0
            effort = np.linalg.norm(self.curr_action)/(self.max_actuation*np.array(self.curr_action).shape[0])

        

        reward = self.w_imitate*(position_r * com_r) + self.w_effort*(1-effort)

        # print('Reward Before: ', (position_r * com_r * (foot_l + foot_r)))
        # print('Reward After: ', (position_r * com_r * (foot_l + foot_r))*(2000-effort)/1500)
    ...
    def get_all_activations_posvel(self):
        ...
        for name in self.osim_model.coordinate_names:
            positions.append(observations['coordinate_pos'][name])
            velocities.append(observations['coordinate_vel'][name])

        if self.osim_model.is_muscle_model:
            for muscle in self.osim_model.muscle_names:
                activations.append(observations['muscles'][muscle]['activation'])

        return activations, positions, velocities

    def get_prescribed_force(self):
        forces = []
        state = self.osim_model.calc_forces_info()
        forces.append(state['forces']['prescribedforce'][0])
