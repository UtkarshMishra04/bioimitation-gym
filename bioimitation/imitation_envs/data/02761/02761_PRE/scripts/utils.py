...
def interpolate_data_table(data_table):
    """Ensure that columns in OpenSim::DataTable do not contain missing values."""
    for label in data_table.getColumnLabels():
        data = data_table.updDependentColumn(label)
        v = vector_vec3_to_nparray(data)
        v = pd.DataFrame(v).interpolate(limit_direction="both", kind="cubic").to_numpy()
        for i in range(v.shape[0]):
            data[i] = opensim.Vec3(v[i, 0], v[i ,1], v[i, 2])    
