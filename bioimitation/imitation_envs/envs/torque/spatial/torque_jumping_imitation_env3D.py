...
class TorqueJumpingImitationEnv3D(OsimEnv):
    def __init__(self,
                 config
                 ):
        ...
        super(TorqueJumpingImitationEnv3D,
    ...
    def step(self, action, obs_as_dict=False):
        ...
        if np.any(np.isnan(action)):
            ...
            v = v[[0, 4, 5, 6, 8, 8, 9, 10, 11, 12, 13]]

            action_x = action[:11]

            Kp = np.divide(np.diag([100, 100, 100, 100, 100, 50, 100, 100, 100, 100, 50]), 1)
            Kv = np.divide(np.diag([5, 5, 5, 5, 5, 2, 5, 5, 5, 5, 2]), 1)
        ...
        return super(TorqueJumpingImitationEnv3D, self).step(self.curr_action, obs_as_dict)
