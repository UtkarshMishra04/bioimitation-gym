...
class TorqueWalkingImitationEnv2D(OsimEnv):
    def __init__(self,
                 config
                 ):
        ...
        super(TorqueWalkingImitationEnv2D,
    ...
    def step(self, action, obs_as_dict=False):
        ...
        return super(TorqueWalkingImitationEnv2D, self).step(self.curr_action, obs_as_dict)
    ...
    def is_done(self):
        ...
        if torso_pos[1] < terminal_height:
            return True
        elif np.max(np.abs(force_limit)) > 1000:
            return True
        elif np.max(np.abs(acceleration)) > 100000:
            return True
