...
class MuscleWalkingImitationEnv2D(OsimEnv):
    def __init__(self,
                 config
                 ):
        ...
        super(MuscleWalkingImitationEnv2D,
    ...
    def step(self, action, obs_as_dict=False):
        ...
        return super(MuscleWalkingImitationEnv2D, self).step(self.curr_action, obs_as_dict)
    ...
    def is_done(self):
        ...
        if torso_pos[1] < terminal_height:
            ...
            return True
        elif np.max(np.abs(acceleration)) > 10000:
            return True
