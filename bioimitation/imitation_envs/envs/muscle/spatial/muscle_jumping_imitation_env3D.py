...
class MuscleJumpingImitationEnv3D(OsimEnv):
    def __init__(self,
                 config
                 ):
        ...
        q_desired_file = os.path.join(subject_dir,
                            'jumping_reference_data/task_Kinematics_q.sto')
        u_desired_file = os.path.join(subject_dir,
                            'jumping_reference_data/task_Kinematics_u.sto')
        body_pos_desired_file = os.path.join(
                            subject_dir, 'jumping_reference_data/task_BodyKinematics_pos_global.sto')
        body_vel_desired_file = os.path.join(
                            subject_dir, 'jumping_reference_data/task_BodyKinematics_vel_global.sto')
        ...
        super(MuscleJumpingImitationEnv3D,
    ...
    def step(self, action, obs_as_dict=False):
        ...
        return super(MuscleJumpingImitationEnv3D, self).step(self.curr_action, obs_as_dict)
    ...
    def reset(self, obs_as_dict=False):
        ...
        if self.test:
            ...
            index = random.randint(0, self.cycle)  # it seems with this it is easier to learn
    ...
    def get_state_dict(self):
        ...
        self.pelvis_pos_z = observation['coordinate_pos']['pelvis_tz']

        del observation['coordinate_pos']['pelvis_tx']
        del observation['coordinate_pos']['pelvis_ty']
        del observation['coordinate_pos']['pelvis_tz']
    ...
    def is_done(self):
        ...
        5. abs(foot_r - foot_l) < 0.1 (foot crossing each other)

        '''
        observations = self.state_dict.copy()
        torso_pos = observations['body_pos']['torso']
        foot_r = self.state_dict['body_pos']['calcn_r'][2]
        foot_l = self.state_dict['body_pos']['calcn_l'][2]
    ...
    def get_reward(self):
        ...
        observations['coordinate_pos']['pelvis_tz'] = self.pelvis_pos_z
        ...
        effort_r = np.exp(-effort/max((self.pelvis_pos_y - self.old_pos_pelvisy + 1),1))
        action_r = np.exp(-0.5*np.linalg.norm(self.curr_action - self.last_action))

        reward = (0.5+self.w_imitate)*(position_r * com_r * (foot_l + foot_r)) + self.w_effort*effort_r \
            + self.w_action*action_r

        self.last_action = self.curr_action
        self.old_pos_pelvisy = self.pelvis_pos_y
    ...
    def calc_cost_of_transport(self):
        ...
        slowTwitchFiberRatios = [0.499, 0.55, 0.5, 0.484, 0.546, 0.759, 0.721, 
                                0.484, 0.546, 0.759, 0.721, 0.499, 0.55, 0.5, 
                                0.484, 0.546, 0.759, 0.721, 0.484, 0.546, 0.759, 0.721]
